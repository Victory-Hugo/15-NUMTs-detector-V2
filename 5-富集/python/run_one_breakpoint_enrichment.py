#!/usr/bin/env python3
"""Run one target-region by breakpoint-stratum enrichment task."""
...
def target_to_pseudo_reference(target_bed: str | Path) -> pd.DataFrame:
    ...
    return pd.DataFrame({"Start": df["start"] + offsets, "End": df["end"] + offsets})
...
def run(
    ...
    if max_start <= 0:
        raise ValueError(f"Genome length is too small for flank length {flank_length}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run one nuclear breakpoint flank enrichment task.")
