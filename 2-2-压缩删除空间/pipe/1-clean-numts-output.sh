#!/usr/bin/env bash
# pipe/1-clean-numts-output.sh
# 清理 1-NUMTs 输出目录：删除可再生的下游文件，并把 *_all_regions.bwa.sam 压缩为 BAM。
# 将样本切块后生成 LSF 作业脚本，按需提交到队列（不在前台执行清理）。
#
# 用法：
#   bash pipe/1-clean-numts-output.sh                       # 按 conf 生成作业脚本（默认 dry-run，不提交）
#   bash pipe/1-clean-numts-output.sh --submit              # 生成并提交
#   bash pipe/1-clean-numts-output.sh --dry-run false --limit 50 --submit
#   bash pipe/1-clean-numts-output.sh --collect             # 合并各作业产出的样本报告
set -euo pipefail

SCRIPT_NAME="$(basename "$0")"
STEP_NAME="1-clean-numts-output"
...
COMPRESS_ENABLED="$CFG_COMPRESS_ENABLED"
SOURCE_SUFFIX="$CFG_COMPRESS_SOURCE_SUFFIX"
TARGET_SUFFIX="$CFG_COMPRESS_TARGET_SUFFIX"
COMPRESS_THREADS="$CFG_COMPRESS_THREADS"
VERIFY="$CFG_COMPRESS_VERIFY"
...
WORKER="${PROJECT_DIR}/script/clean_numts_sample.sh"
CHUNK_DIR="${TEMP_DIR}/chunks"
JOB_DIR="${TEMP_DIR}/jobs"
REPORT_DIR="${TEMP_DIR}/reports"
SAMPLE_LIST="${TEMP_DIR}/samples.txt"
MERGED_REPORT="${OUTPUT_RESULT}/1-1-clean_numts_report.tsv"
SUMMARY_REPORT="${OUTPUT_RESULT}/1-2-clean_numts_summary.tsv"
...
# ------------------------------------------------------------------ #
# --collect：把每样本报告合并为长表，并按 action×category 汇总
# ------------------------------------------------------------------ #
...
if [[ "$COMPRESS_ENABLED" == "true" && ! -x "$SAMTOOLS_BIN" ]]; then
    echo "[ERROR] samtools 不可执行: $SAMTOOLS_BIN" >&2; exit 1
fi
