...
if command -v rsvg-convert >/dev/null 2>&1; then
    rsvg-convert -a --dpi-x "$PNG_DPI" --dpi-y "$PNG_DPI" --width "$PNG_WIDTH" --height "$PNG_HEIGHT" -o "$OUTPUT_PNG" "$OUTPUT_SVG"
    rsvg-convert -a --dpi-x "$PNG_DPI" --dpi-y "$PNG_DPI" --width "$PNG_WIDTH" --height "$PNG_HEIGHT" -o "$OUTPUT_NUMT_HEATMAP_PNG" "$OUTPUT_NUMT_HEATMAP"
elif command -v inkscape >/dev/null 2>&1; then
    inkscape "$OUTPUT_SVG" --export-type=png --export-filename="$OUTPUT_PNG" --export-dpi="$PNG_DPI" --export-width="$PNG_WIDTH" --export-height="$PNG_HEIGHT" >/dev/null 2>&1
    inkscape "$OUTPUT_NUMT_HEATMAP" --export-type=png --export-filename="$OUTPUT_NUMT_HEATMAP_PNG" --export-dpi="$PNG_DPI" --export-width="$PNG_WIDTH" --export-height="$PNG_HEIGHT" >/dev/null 2>&1
else
