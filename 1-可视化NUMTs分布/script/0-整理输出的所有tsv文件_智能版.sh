...
PYTHON_SCRIPT_LDY="/mnt/f/OneDrive/文档（科研）/脚本/Download/15-NUMTs-detector-V2/1-可视化NUMTs分布/script/0-打包_LDY.py" #* 新版本脚本
...
INPUT_CSV_DIR="/mnt/c/Users/Administrator/Desktop/share/home/grp-wangyf/luolintao/1-luolintao/2-NUMTs/1-整理/output" #* 里面存放了所有的tsv文件
OUTPUT_DIR="/mnt/c/Users/Administrator/Desktop/SLE_完成_output" #* 输出目录，存放合并后的tsv文件
TAR_GZ_PATH="/mnt/c/Users/Administrator/Desktop/SLE完成.tar.gz" #* 输出的tar.gz压缩文件路径
...
detect_file_version() {
    ...
    检测输入目录中的文件版本
    返回值: 
      - 0 表示旧版本 (*.Breakpoints.tsv, *.mt.disc.sam.breakpointINPUT.tsv 等)
      - 1 表示新版本 (*.ConfidentBreakpoints.tsv, *.AllBreakpoints.tsv 等)
      - 2 表示混合版本 (两种格式都有)
    """
    local input_dir="$1"
...
case $VERSION in
    ...
    1)
        ...
        ;;
    2)
        echo "检测结果: 混合版本文件 (包含旧版本和新版本)"
        echo "警告: 目录中同时包含两种版本的文件!"
...
if [ "$VERSION" -eq 1 ] || ([ "$VERSION" -eq 2 ] && [ "$SCRIPT_NAME" = "0-打包_LDY.py" ]); then
    # 新版本没有cluster.summary，检查是否有cluster文件
...
# 显示统计信息
echo "统计信息:"
echo "- 输入目录: $INPUT_CSV_DIR"
echo "- 输出目录: $OUTPUT_DIR"
echo "- 压缩文件: $TAR_GZ_PATH"
echo "- 使用脚本: $SCRIPT_NAME"
echo ""
echo "输出文件列表:"
ls -lh "$OUTPUT_DIR"/*.tsv 2>/dev/null || echo "未找到输出文件"
echo ""
...
case $VERSION in
    0)
        ...
        ;;
    1)
        echo "  新版本 (LDY版本) - 支持的文件类型:"
        echo "  - *ConfidentBreakpoints.tsv"
        echo "  - *AllBreakpoints.tsv"
        echo "  - *.mt.disc.sam.breakpointINPUT.tsv"
        echo "  - *.mt.disc.sam.cluster.summary.tsv"
        echo "  - *.mt.disc.sam.cluster.tsv"
        ;;
    2)
        echo "  混合版本 - 同时包含旧版本和新版本文件"
        ;;
esac
...
echo "执行示例:"
echo "  # 手动执行旧版本脚本"
echo "  python 0-打包.py <输入目录> <输出目录> <压缩文件路径>"
echo ""
echo "  # 手动执行新版本脚本"
echo "  python 0-打包_LDY.py <输入目录> <输出目录> <压缩文件路径>"
echo ""
echo "  # 自动检测版本执行"
echo "  bash 0-整理输出的所有tsv文件.sh"
echo ""
echo "==============================================="
