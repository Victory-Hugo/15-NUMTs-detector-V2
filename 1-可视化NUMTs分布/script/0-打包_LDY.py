#!/usr/bin/env python3
"""
TSV文件合并与压缩工具 - 0-打包_LDY.py (新版本)

该脚本将合并指定目录下的TSV文件并压缩原始文件：
1. *ConfidentBreakpoints.tsv - 所有个体的可信断点文件
2. *AllBreakpoints.tsv - 所有个体的所有断点文件
3. *.mt.disc.sam.breakpointINPUT.tsv - 所有个体的断点输入文件  
4. *.mt.disc.sam.cluster.summary.tsv - 所有个体的聚类摘要文件
5. *.mt.disc.sam.cluster.tsv - 所有个体的聚类详细文件

用法:
python 0-打包_LDY.py <输入目录> <输出目录> <压缩文件路径>

示例:
python 0-打包_LDY.py ./完成1/tsv ./完成1/merged_output ./完成1/original_files.tar.gz
"""
...
def merge_files_by_type(input_dir, output_dir):
    ...
    # 定义文件类型模式和输出文件名 (新版本格式)
    file_patterns = {
        'confident_breakpoints': {
            'pattern': '*ConfidentBreakpoints.tsv',
            'output': 'all_individuals_ConfidentBreakpoints.tsv',
            'has_header': False
        },
        'all_breakpoints': {
            'pattern': '*AllBreakpoints.tsv',
            'output': 'all_individuals_AllBreakpoints.tsv',
            'has_header': False
        },
        'breakpoint_input': {
...
def print_usage():
    """打印使用说明"""
    print_header("使用说明 (LDY新版本)")
    print_colored("用法:", Colors.BOLD)
    print_colored("  python 0-打包_LDY.py <输入目录> <输出目录> <压缩文件路径>", Colors.WHITE)
    print_colored("\n参数说明:", Colors.BOLD)
    ...
    print_colored("\n新版本支持的文件类型:", Colors.BOLD)
    print_colored("  - *ConfidentBreakpoints.tsv", Colors.WHITE)
    print_colored("  - *AllBreakpoints.tsv", Colors.WHITE)
    print_colored("  - *.mt.disc.sam.breakpointINPUT.tsv", Colors.WHITE)
    print_colored("  - *.mt.disc.sam.cluster.summary.tsv", Colors.WHITE)
    print_colored("  - *.mt.disc.sam.cluster.tsv", Colors.WHITE)
    print_colored("\n示例:", Colors.BOLD)
    print_colored("  python 0-打包_LDY.py ./完成1/tsv ./完成1/merged_output ./完成1/original_files.tar.gz", Colors.WHITE)
...
def main():
    ...
    print_header("TSV文件合并与压缩工具 (LDY新版本)")
